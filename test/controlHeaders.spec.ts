...
test("cache control", async () => {
  ...
  const staleResp = await workerFetch(ctx, new Request(url, {
    headers: {
...
test("content disposition without specifying filename", async () => {
  ...
  expect((await workerFetch(ctx, url)).headers.get("Content-Disposition"))
    .toStrictEqual("inline")
  expect((await workerFetch(ctx, `${url}?a`)).headers.get("Content-Disposition"))
    .toStrictEqual("attachment")

  expect((await workerFetch(ctx, `${url}/${filename}`)).headers.get("Content-Disposition"))
    .toStrictEqual(`inline; filename*=UTF-8''${filename}`)
  expect((await workerFetch(ctx, `${url}/${filename}?a`)).headers.get("Content-Disposition"))
...
test("content disposition with specifying filename", async () => {
  ...
  expect((await workerFetch(ctx, `${url}/${altFilename}`)).headers.get("Content-Disposition"))
    .toStrictEqual(`inline; filename*=UTF-8''${altFilename}`)
  expect((await workerFetch(ctx, `${url}/${altFilename}?a`)).headers.get("Content-Disposition"))
